...
class ReceptionAgent(BaseAgent):
    ...
    async def _handle_initial_response(self, message: str, whatsapp_id: str) -> Dict[str, Any]:
        # Clasificar usando ChatGPT-4o mini: ¿Es saludo simple o necesidad específica?
        from app.config import settings

        if settings.fixed_flow_mode:
            # Modo simple: Cualquier respuesta → Pedir nombre
            ...
            # Modo LLM: Clasificar inteligentemente
            return await self._classify_and_transfer(message, "Usuario", whatsapp_id)
    ...
    async def _classify_and_transfer(self, message: str, customer_name: str, whatsapp_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.llm_service or not self.llm_service.api_client.initialized:
                ...
                "pregunta" - Si es una consulta informativa (precios, servicios, información, cómo funciona, qué ofrecen, etc.)
                "necesidad" - Si expresa una necesidad específica de compra/venta (quiero comprar, necesito casa, busco apartamento, etc.)

                Ejemplos:
                - "¿Qué precios manejan?" → pregunta
                - "¿Cuáles son sus servicios?" → pregunta
                - "Quiero comprar casa" → necesidad
                - "Busco apartamento" → necesidad
